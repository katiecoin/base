...
//
// Genuinely new at V2, or diverging from V1's pinned behavior (updateMultiplier now emits both the
// legacy and ERC-8056 events; announce's inner updateMultiplier call therefore emits a different
// sequence too).
...
fn golden_read_operator_role_and_wad() {
    ...
    .unwrap();
    assert_eq!(max_ui_multiplier, Bytes::from(U256::from(u128::MAX).abi_encode()));
...
fn golden_read_multiplier_and_scaled_balances() {
    ...
    .unwrap();
    assert_eq!(from_ui, Bytes::from(u(100).abi_encode()));
...
fn golden_update_ui_multiplier_schedules_pending() {
    ...
    assert_root("update_ui_multiplier", s, ROOT_SET_UI_MULTIPLIER);
}
...
#[test]
fn golden_update_ui_multiplier_reverts_pending_update_exists() {
    let mut s = fresh();
    seed(&mut s, |t| give_role(t, operator_role(), ALICE));
    warp(&mut s, u(1));
    op(
...
fn golden_update_ui_multiplier_folds_matured_pending_before_rescheduling() {
    ...
    assert_root("update_ui_multiplier_folds_matured", s, ROOT_SET_UI_MULTIPLIER_FOLDS_MATURED);
}
...
fn golden_cancel_ui_multiplier_update() {
    ...
    assert_root("cancel_ui_multiplier_update", s, ROOT_CANCEL_SCHEDULED_MULTIPLIER);
}
...
#[test]
fn golden_supports_interface_false_for_unrelated_extension_id() {
    let mut s = fresh();
    let out = op(
        &mut s,
        ALICE,
        FakePolicyAccounting::new(),
        IB20Asset::supportsInterfaceCall {
            interfaceId: alloy_primitives::FixedBytes::new([0xff, 0xff, 0xff, 0xff]),
        }
        .abi_encode(),
    )
    .unwrap();
...
fn golden_announce_emits_and_runs_internal_calls() {
    let mut s = fresh();
    seed(&mut s, |t| give_role(t, operator_role(), ALICE));
    // The inner `updateMultiplier` call's emitted events carry `now`, so the
    ...
    let events = s.get_events(TOKEN);
    // Announcement, MultiplierUpdated, UIMultiplierUpdated, EndAnnouncement.
    assert_eq!(
        events[events.len() - 4],
...
fn v2_op_coverage_checklist(call: IB20::IB20Calls, ext: IB20Asset::IB20AssetCalls) {
    ...
    match ext {
        ...
        SC::OPERATOR_ROLE(_) | SC::WAD_PRECISION(_) | SC::MAX_UI_MULTIPLIER(_) => {
            covered(&[golden_read_operator_role_and_wad])
        }
        SC::multiplier(_)
        | SC::toScaledBalance(_)
        | SC::toRawBalance(_)
        | SC::scaledBalanceOf(_)
        | SC::toUIAmount(_)
        | SC::fromUIAmount(_) => covered(&[golden_read_multiplier_and_scaled_balances]),
        ...
        SC::updateUIMultiplier(_) => covered(&[
            ...
            golden_update_ui_multiplier_reverts_pending_update_exists,
        ...
        SC::supportsInterface(_) => covered(&[
            ...
            golden_supports_interface_false_for_unrelated_extension_id,
        ]),
    }
}
